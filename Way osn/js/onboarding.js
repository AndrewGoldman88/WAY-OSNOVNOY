...
function finishOnboarding(skipped) {
    ...
    // При первом прохождении после онбординга сначала предлагаем аккаунт.
    // Повторный запуск анкеты из настроек сразу возвращает в приложение.
    if (!obReplay && !state.accountChoiceDone) {
        if (typeof showAccountIntro === 'function') {
            showAccountIntro();
            return;
        }
    }
