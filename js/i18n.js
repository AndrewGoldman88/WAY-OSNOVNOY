...
const TRANSLATIONS = {
    ru: {
        ...
        account_signed_in_toast: 'Вход выполнен — синхронизирую прогресс',
        account_signin_failed: msg => `Не вышло: ${msg}`,
        auth_err_timeout: 'Сервер не ответил за 20 секунд. Проверь интернет-соединение и попробуй ещё раз.',
        auth_err_network: 'Не удалось подключиться к серверу. Проверь интернет-соединение.',
        auth_err_not_ready: 'Сервис аккаунтов ещё не загрузился. Обнови страницу и попробуй снова.',
        ...
        account_delete_btn: 'Удалить аккаунт и все данные',
        account_delete_title: 'Удалить аккаунт?',
        account_delete_sub: 'Это действие необратимо. Будут удалены аккаунт, облачный прогресс и локальные данные приложения на этом устройстве.',
        account_delete_cancel: 'Отмена',
        account_delete_confirm: 'Удалить всё',
        account_deleting: 'Удаляю аккаунт и данные...',
        account_deleted_toast: 'Аккаунт и данные удалены',
        account_delete_failed: msg => `Не удалось удалить аккаунт: ${msg}`,
    ...
    en: {
        ...
        account_signed_in_toast: 'Signed in — syncing your progress',
        account_signin_failed: msg => `Didn't work: ${msg}`,
        auth_err_timeout: 'The server did not respond within 20 seconds. Check your internet connection and try again.',
        auth_err_network: 'Could not connect to the server. Check your internet connection.',
        auth_err_not_ready: 'The account service is still loading. Refresh the page and try again.',
        ...
        account_delete_btn: 'Delete account and all data',
        account_delete_title: 'Delete account?',
        account_delete_sub: 'This action cannot be undone. Your account, cloud progress and local app data on this device will be deleted.',
        account_delete_cancel: 'Cancel',
        account_delete_confirm: 'Delete everything',
        account_deleting: 'Deleting account and data...',
        account_deleted_toast: 'Account and data deleted',
        account_delete_failed: msg => `Could not delete account: ${msg}`,
...
function applyLang() {
    ...
    if (accDeleteBtn) accDeleteBtn.textContent = L('account_delete_btn');

    const deleteAccountH = document.querySelector('#modal-delete-account h2');
    if (deleteAccountH) deleteAccountH.textContent = L('account_delete_title');
    const deleteAccountSub = document.querySelector('#modal-delete-account .sub');
    if (deleteAccountSub) deleteAccountSub.textContent = L('account_delete_sub');
    const deleteAccountCancel = document.querySelector('#modal-delete-account .btn-ghost');
    if (deleteAccountCancel) deleteAccountCancel.textContent = L('account_delete_cancel');
    const deleteAccountConfirm = document.getElementById('deleteAccountConfirmBtn');
    if (deleteAccountConfirm) deleteAccountConfirm.textContent = L('account_delete_confirm');
